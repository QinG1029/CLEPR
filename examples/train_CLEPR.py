...
from clepr.trainers import MMTTrainerCL
from clepr.evaluators import Evaluator, extract_features
from clepr.utils.data import IterLoader
from clepr.utils.data import transforms as T
from clepr.utils.data.sampler import RandomMultipleGallerySampler
from clepr.utils.data.preprocessor import Preprocessor
from clepr.utils.logging import Logger
from clepr.utils.serialization import load_checkpoint, save_checkpoint, copy_state_dict
...
def get_train_loader(dataset, height, width, batch_size, workers,
    ...
    for i in range(len(train_set)):#每个item是一个tuple，(img_path, pid, camid) pid是person_id,camid是camera_id
        train_set[i] = list(train_set[i])#转换成list，因为tuple是不可变的
        train_set[i][1] = int(select_samples_labels[i])#将pid改成select_samples_labels[i]，即新的伪标签，因为原始数据集是有标签的，这个工作是伪标签所以要将原始ground truth标签改成伪标签
        train_set[i] = tuple(train_set[i])#再转换成tuple
    ...
    return train_loader, select_samples_inds, select_samples_labels#注意，这里的train_loader的标签是伪标签，不是原始ground truth标签，而且已经根据uncertainty进行了筛选
...
def main_worker(args):
    ...
    if args.fast_kmeans:
        ...
        #km = KMeans(n_clusters=args.num_clusters, random_state=args.seed, n_jobs=8,max_iter=300).fit(cf)
        km = KMeans(n_clusters=args.num_clusters, random_state=args.seed, max_iter=300).fit(cf)
    ...
    def scheduler(t, T, p0):
        au = (t / T) * (np.exp((3 / 2) * (1 - p0)) - 1)
        return np.clip(p0 + (2 / 3) * (au * (6 + au) / (6 + 4 * au)), 0, 1)

    for epoch in tqdm(range(args.epochs)):
        # pt = scheduler(epoch, args.epochs-1, start_percentage, h=1.5)
        pt = scheduler(t=epoch, T=args.epochs-1, p0=start_percentage)
        ...
        # Trainer
        trainer = MMTTrainerCL(model_1, model_2, model_1_ema, model_2_ema,
                            num_cluster=args.num_clusters, alpha=args.alpha, temperature=args.contrastive_learning_temperature)
        trainer.train(epoch, train_loader_target, optimizer,
                    ce_soft_weight=args.soft_ce_weight, tri_soft_weight=args.soft_tri_weight,
                    print_freq=args.print_freq, train_iters=len(train_loader_target),contrast_weight=args.contrast_weight)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--contrastive_learning_temperature', type=float, default=0.1)
    main()
